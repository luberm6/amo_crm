...
| Inventory data available | **YES** (2 lines + 2 extensions) |
...
Extensions (внутренние номера) — **есть** (API вернул 2 usable records).  
Webhook credentials не заданы — webhook verification по-прежнему не hardened.
...
| `MANGO_FROM_EXT` | ❌ Пуст | Runtime теперь может auto-discover extension, но pinned config всё ещё предпочтительнее |
...
### 4.2 Расширения / сотрудники (`/config/users/request`) — 2 объекта

| extension | name | outgoing_line |
|---|---|---|
...
- repeated `config/users/request` calls can still hit `429`
- runtime now needs cache/fallback when it auto-discovers a source extension
...
| Extension / внутренний номер | ✅ Присутствует, но repeated polling is rate-limited |
...
| `MANGO_FROM_EXT` | ⚠️ Не задан — runtime can auto-discover, but explicit pinning is still recommended |
...
| POST `/config/users/request` | ✅ HTTP 200, 2 объекта |
...
| Исходящий звонок (originate) | Реальный звонок не делаем в аудите; нужен live smoke через Mango callback |
