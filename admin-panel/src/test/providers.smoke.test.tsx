...
describe('providers page smoke', () => {
  ...
  it('renders SaaS-style Mango status, inventory, and routing hints', async () => {
    ...
    expect(screen.getAllByText(/Recommended for AI/i).length).toBeGreaterThanOrEqual(1)
    expect(screen.getAllByText(/Bound agent:/i).length).toBeGreaterThanOrEqual(1)
    expect(screen.getByText(/Mango Status/i)).toBeInTheDocument()
    expect(screen.getByText(/Connected → Synced → Bound → Live-ready/i)).toBeInTheDocument()
    ...
    expect(screen.getAllByText(/^Live-ready$/i).length).toBeGreaterThanOrEqual(1)
    expect(screen.getByText(/^API key status$/i)).toBeInTheDocument()
    ...
    expect(screen.getAllByText(/Sync numbers from Mango/i).length).toBeGreaterThanOrEqual(1)
    expect(screen.getByText(/Bind number in Agent settings/i)).toBeInTheDocument()
    expect(screen.queryByText(/No numbers synced yet/i)).not.toBeInTheDocument()
    ...
    expect(screen.getAllByRole('link', { name: /Open bound agent/i })[0]).toHaveAttribute('href', '/agents/agent-1?mango_line=405622036&from=providers')
    expect(screen.getByText(/Last sync status/i)).toBeInTheDocument()
    expect(screen.getByText(/Inventory is visible/i)).toBeInTheDocument()
    expect(screen.getByText(/Numbers are already synced/i)).toBeInTheDocument()
    expect(screen.getAllByRole('button', { name: /Refresh inventory/i }).length).toBeGreaterThanOrEqual(1)
    ...
    expect(screen.getByText(/Review current agent binding/i)).toBeInTheDocument()
    expect(screen.getByText(/Open the bound agent to confirm telephony, voice, and prompt settings/i)).toBeInTheDocument()
    ...
    expect(screen.getByText(/What happens here:/i)).toBeInTheDocument()
    ...
    expect(screen.getByText(/raw lines count/i)).toBeInTheDocument()
    expect(screen.getByText(/last sync response/i)).toBeInTheDocument()
    expect(screen.getByText(/readiness flags/i)).toBeInTheDocument()
    ...
    expect(screen.getByRole('link', { name: /Go bind this line now/i })).toHaveAttribute('href', '/agents?source=mango&line=405519147')
